...
colour_linked(Pix,Frame):-
	frame(S1,Frame),
	F2 is Frame + 1,
	frame(S2,F2),
	member(colour(Pix,C),S1),
...
change_colour(Pix,Frame):-
	frame(S1,Frame),
	F2 is Frame + 1,
	frame(S2,F2),
	member(colour(Pix,C1),S1),
	member(colour(Pix,C2),S2),
	C1 \= C2.
*/

change_colour(P1,T):-
	transitions(Q,_,_) is T,
	member(change_colour(P1,_),Q).

colour_linked(P1,P2,T):-
	transitions(Q,S1,S2) is T,
	state(S1_1,S1),
	state(S2_1,S2),
	member(colour(P1,C),S1_1),
	member(change_colour(P2,C),Q).
